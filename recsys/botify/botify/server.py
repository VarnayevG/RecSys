...
from botify.recommenders.contextual_updated import СontextualUpdated
...
tracks_with_updated_recs_redis = Redis(app, config_prefix="REDIS_RECOMMENDATIONS_NN")
...
catalog = Catalog(app).load(app.config["TRACKS_CATALOG"],
                            ...
                            app.config["RECOMMENDATIONS_NN_FILE_PATH"])
catalog.upload_tracks(tracks_redis.connection,
                      tracks_with_diverse_recs_redis.connection,
                      tracks_with_updated_recs_redis.connection)
...
class NextTrack(Resource):
    ...
    def post(self, user: int):
        ...
        treatment = Experiments.RECOMMENDERS_UPDATED.assign(user)
        if treatment == Treatment.T1:
            recommender = СontextualUpdated(tracks_with_updated_recs_redis.connection,
                                            recommendations_svd_redis.connection,
                                            catalog,
                                            tracks_redis.connection,
                                            artists_redis.connection,
                                            catalog.top_tracks[:100]
                                            )
            recommendation = recommender.recommend_next(user, args.track, args.time, HISTORY)
        else:
